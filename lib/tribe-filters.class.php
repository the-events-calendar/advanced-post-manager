...
class Tribe_Filters {
	...
	public function output_form() {
		...
		echo '<h4>'.__('Active Filters', 'tribe-apm').'</h4>';
	...
	protected function saved_filters_dropdown() {
		...
		if ( ! empty( $this->saved_active ) && isset( $this->saved_active) ) {
			...
			$sel .= '<input type="submit" name="tribe-update-saved-filter" value="Update Filter" class="button-secondary" />';
			$sel .= '<input type="submit" name="tribe-delete-saved-filter" value="Delete Filter" class="button-secondary" />';
			$sel .= '</span>';
		}
